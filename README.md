...
**Vigía** (Spanish for *watchman*) is a lightweight monitoring service for my homelab.
...
Sustained outages produce exactly one alert. Recovery is announced on the first
successful check after `DOWN`.
...
Without them, Vigía still runs and logs alerts instead of sending them — useful in
development.

## Design decisions

- **Alerts only on transitions.** The state machine is the core of the project: it
  exists to guarantee *zero duplicate notifications* during a sustained outage.
...
- **Ping shells out to iputils** instead of using `InetAddress.isReachable`, which
  silently degrades to a TCP probe without raw-socket privileges.
- **Ping targets starting with `-` are rejected** so a config value can never be
  smuggled in as a ping flag.
- **A crashing check counts as a failure** and can never kill its schedule: a task
